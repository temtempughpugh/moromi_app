...
export default function JosoHyokaComponent({ 
  ...
  const getHyoka = (jungoId: string): JosoHyoka => {
    ...
    return {
      ...
      ratingComment: localUpdate?.ratingComment ?? existing?.ratingComment ?? '',
  ...
  const updateRating = (jungoId: string, rating: 'S' | 'A' | 'B' | 'C' | 'D' | null) => {
    ...
    const newTimer = setTimeout(async () => {
      ...
      await saveJosoHyoka({
        ...
        jungoId,
        rating,
        ratingComment: hyoka.ratingComment,
  ...
  // 評価コメントを更新
  const updateRatingComment = (jungoId: string, comment: string) => {
    const key = `${currentBY}-${jungoId}`;
    ...
    const existingTimer = debounceTimers.current.get(`ratingComment-${key}`);
    if (existingTimer) clearTimeout(existingTimer);
    ...
    const newTimer = setTimeout(async () => {
      ...
      debounceTimers.current.delete(`ratingComment-${key}`);
    }, 1500);

    debounceTimers.current.set(`ratingComment-${key}`, newTimer);
  };
  ...
  const handleRatingCommentBlur = async (jungoId: string) => {
    const key = `${currentBY}-${jungoId}`;
    const existingTimer = debounceTimers.current.get(`ratingComment-${key}`);
    if (existingTimer) {
      clearTimeout(existingTimer);
          ...
          <table className="w-full">
            <thead className="bg-slate-100 sticky top-0">
              <tr>
                ...
                <th className="px-2 py-3 text-center text-sm font-bold border-r">評価</th>
                  ...
                  <tr key={moromi.jungoId} className="border-b hover:bg-slate-50">
                    ...
                    <td className="px-2 py-2 border-r">
                      <div className="flex flex-col gap-2">
                        ...
                        <textarea
                          value={hyoka.ratingComment || ''}
                          onChange={(e) => updateRatingComment(moromi.jungoId, e.target.value)}
                          onBlur={() => handleRatingCommentBlur(moromi.jungoId)}
                          placeholder="評価コメント(100文字)"
                          maxLength={100}
                          rows={4}
                          className="w-40 px-2 py-1 text-xs border rounded resize-none leading-tight"
                        />
                      </div>
                    </td>
                    {activeStaff.map(staff => (
                      <td key={staff.id} className="px-2 py-2 border-r">
                        <div className="flex flex-col gap-2">
                          <select
                            value={hyoka.staffComments[`${staff.id}_rating`] || ''}
                            onChange={(e) => {
                              const value = e.target.value;
                              updateComment(
                                moromi.jungoId, 
                            ...
                            onBlur={() => handleCommentBlur(moromi.jungoId, `${staff.id}_rating`)}
